...
```python
def extract_with_detailed_progress(self, file_path: str, progress_callback=None):
    """Show detailed progress during extraction"""
    
    if progress_callback:
        progress_callback("Initializing decoder...", 5)
    ...
    with open(file_path, 'rb') as f:
        ...
        while True:
            ...
            if progress_callback:
                progress = 10 + (20 * bytes_read // file_size)
                progress_callback(f"Reading file... {bytes_read//1024//1024}MB", progress)
    
    # Phase 2: Parse data
    if progress_callback:
        progress_callback("Parsing GPS records...", 35)
    
    # ... parsing logic ...
    
    # Phase 3: Validate data
    if progress_callback:
        progress_callback("Validating coordinates...", 80)
...
```python
def validate_gps_entry(self, lat: float, lon: float, timestamp: Any) -> bool:
    """Validate GPS data before creating entry"""
    ...
    try:
        if isinstance(timestamp, (int, float)):
            # Check reasonable timestamp range (2000-2050)
            if not (946684800 <= timestamp <= 2524608000):
                return False
...
```python
def process_large_file(self, file_path: str, progress_callback=None):
    ...
    with open(file_path, 'rb') as f:
        ...
        record_count = struct.unpack('<I', header[16:20])[0]
        ...
        while records_processed < record_count:
            ...
            for i in range(min(batch_size, record_count - records_processed)):
                record = f.read(32)  # Assuming 32-byte records
            ...
            # Update progress
            if progress_callback:
                progress = 30 + (60 * records_processed // record_count)
                progress_callback(f"Processed {records_processed}/{record_count} records", progress)
